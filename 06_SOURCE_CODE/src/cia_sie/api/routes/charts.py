...
from cia_sie.dal.repositories import ChartRepository, SiloRepository
...
async def create_chart(
    ...
    """
    # Validate silo exists before creating chart
    silo_repo = SiloRepository(db)
    existing_silo = await silo_repo.get_by_id(str(data.silo_id))
    if not existing_silo:
        raise HTTPException(status_code=404, detail=f"Silo with id {data.silo_id} not found")
