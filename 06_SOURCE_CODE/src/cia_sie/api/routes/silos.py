...
from cia_sie.dal.repositories import InstrumentRepository, SiloRepository
...
async def create_silo(
    ...
    Create a new silo.
    """
    # Validate instrument exists before creating silo
    instrument_repo = InstrumentRepository(db)
    existing_instrument = await instrument_repo.get_by_id(str(data.instrument_id))
    if not existing_instrument:
        raise HTTPException(status_code=404, detail=f"Instrument with id {data.instrument_id} not found")
