        ...
        async with httpx.AsyncClient() as client:
            ...
            return HTMLResponse(
                ...
                <body style="font-family: Arial, sans-serif; padding: 40px; text-align: center;">
                    ...
                    <br>
                    <p style="color: #666;">You may close this window and return to your application.</p>
                </body>
                </html>
                """
            )
...
async def get_kite_holdings():
    """
    Fetch holdings from Kite account.
    
    Returns list of stocks you own in your demat account.
    """
    registry = get_registry()
    _ensure_adapters_registered()
    ...
    if not adapter or not adapter.is_connected:
        raise HTTPException(status_code=400, detail="Not connected to Kite. Please login first.")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            response = await client.get(
                ...
                headers={
                    "Authorization": f"token {settings.kite_api_key}:{adapter._access_token}",
                    "X-Kite-Version": "3",
                },
            )
            
            if response.status_code != 200:
                logger.error(f"Kite holdings API error: {response.text}")
                raise HTTPException(status_code=response.status_code, detail=response.json().get("message", "API error"))
...
async def get_kite_positions():
    ...
    Returns current day's positions (intraday + delivery).
    """
    registry = get_registry()
    _ensure_adapters_registered()
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error(f"Kite positions API error: {response.text}")
                raise HTTPException(status_code=response.status_code, detail=response.json().get("message", "API error"))
...
async def get_kite_profile():
    """
    Fetch user profile from Kite account.
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.error(f"Kite profile API error: {response.text}")
                raise HTTPException(status_code=response.status_code, detail=response.json().get("message", "API error"))
