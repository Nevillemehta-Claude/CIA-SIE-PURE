...
def create_app() -> FastAPI:
    ...
    # CORS: Use explicit origins from environment configuration
    # Empty list = no CORS (pure API mode, ideal for backend-only deployment)
    cors_origins = [o for o in settings.cors_origins_list if o]  # Filter empty strings
    if cors_origins:
        logger.info(f"CORS configured for origins: {cors_origins}")
    else:
        logger.info("CORS: No origins configured (API-only mode)")
